...
RAMEN_DIR="${RAMEN_DIR:-$(cd "$PROJECT_DIR/../../../" 2>/dev/null && pwd || echo "")}"
...
ensure_crds() {
    ...
    # Look for CRDs in the Ramen repo
    if [[ -d "$RAMEN_DIR/hack/test" ]]; then
        crds_dir="$RAMEN_DIR/hack/test"
    else
        log_warn "Ramen repo not found at $RAMEN_DIR"
        log_warn "Set RAMEN_DIR to the path of the Ramen repo, or install CRDs manually"
...
deploy_ots() {
    ...
    if [[ -z "$deploy_dir" ]]; then
        log_error "No deploy directory specified."
        log_error "Clone the ramen-ots repo and pass --deploy-dir <path>/scripts/deploy"
        log_error "  git clone https://github.com/dstanley/ramen-ots.git"
        log_error "  $0 --clusters ... --deploy-dir ramen-ots/scripts/deploy"
        exit 1
    fi
